...
use k580_ui::backend::{AppCommand, AppEvent, Emulator, RunMode, spawn_emulator};
use k580_ui::persistence::{ExportOptions, ExportTextSection, ExportXlsxPage};
...
fn import_xlsx_sheet_applies_only_selected_sheet() {
    let first = k580_ui::persistence::ExportModel {
        registers: Vec::new(),
        flags: Vec::new(),
        memory: vec![(0x0100, 0xAA)],
    };
    let second = k580_ui::persistence::ExportModel {
        registers: Vec::new(),
        flags: Vec::new(),
        memory: vec![(0x0200, 0xBB)],
    };
    let path = unique_temp_file("k580-import-pages.xlsx");
    k580_ui::persistence::Exporters::write_xlsx_pages(
        &path,
        &[
...
fn import_txt_section_applies_only_selected_section() {
    ...
    std::fs::write(
        &path,
        k580_ui::persistence::Exporters::to_text_sections(&[
