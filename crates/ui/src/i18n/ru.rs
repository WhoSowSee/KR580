...
pub(super) fn translate(key: Key) -> &'static str {
    ...
    match key {
        ...
        Key::StatusInvalidByteHex => "Неверное шестнадцатеричное значение байта",
        Key::StatusInvalidAddressHex => "Неверный шестнадцатеричный адрес",
        Key::HddImageAttached => "Файл жёсткого диска",
