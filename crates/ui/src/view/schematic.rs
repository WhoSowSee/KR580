    ...
    pub(super) fn schematic_panel(&self) -> Element<'_, Message> {
        ...
        let core_left = column![
            row![
                ...
                schematic_readout(
                    "Регистр флагов",
                    // Пять явно собранных битов в порядке Z S P C AC —
                    // тот же порядок и тот же набор, что у точек-
                    // индикаторов `flag_strip` над верхней шиной. Раньше
                    // здесь стоял `{:06b}` от `flags.to_psw()`, который
                    // вытаскивал шесть младших битов PSW и захватывал
                    // аппаратную единицу в позиции 1 (бит, который чип
                    // всегда читает как `1`). Школьный референсный
                    // эмулятор показывает только пять флаговых битов без
                    // аппаратных констант, и пользователь увидел в наших
                    // `010010` лишний хвост — заодно ширина строки
                    // совпадает с числом точек над ней. Раскладку Z S P C
                    // AC выбрали так, чтобы readout читался слева направо
                    // ровно как зажжённые точки: `flag_strip` рисует Z,
                    // S, P, C, AC именно в этом порядке.
                    format!(
                        "{}{}{}{}{}",
