...
editor `↵` button. Tooltip bodies use `inset_style`; it shares the same
darker `TOKYO_BOARD` fill as the `Регистр состояния` tooltip, so all
hover tips now use one surface tone. The same actions
...
glyph centred inside a `schematic_block_style` chassis (38×38, the
...
tooltips. The chip is a plain `container`, not a `button` — at this
stage the user asked for tooltips only; no per-chip click handler is
wired and a no-op `button` would advertise an interaction the chip
does not have.
...
Интерактивные элементы сохраняют feedback: `schematic_block_button_style`
оставляет resting state прозрачным, но на hover/press поднимает тон до
`TOKYO_SURFACE_3`. Для speed tiers и РОН-chip'ов мультиплексора
используется `schematic_select_button_style`: resting state без
заливки, hover/press — нейтральная серая заливка без цветной рамки,
selected — та же синяя заливка, что у выбранной строки ОЗУ.
...
- the shared upper frame: CPU row (`PC` / `SP` / `T` / clickable
  ...
  `Цикл и такт`, `Внутренние тайминги`, `Сигналы управления`, plus
  the right-aligned central column with `Статус`, same-width `Буфер
  данных` and `Регистр признаков`, the multiplexer, and the
...
peripheral frame. It is a four-position segmented switch inside a
legend-framed `Скорость` panel. The panel is sized to the same bottom
control rhythm as «Выполнение» / «Сброс»: 38 px tall buttons, neutral
outline-only rest state, grey hover fill, and blue selected fill. Each
button wraps its label in a fill-sized centring container, so the text is
centred both horizontally and vertically inside the button frame.
...
Each tier button is a `button` styled with `schematic_select_button_style`.
The default tier (`Medium`) is selected during startup. Resting buttons
are outline-only, hover/press uses a neutral surface fill with no
coloured border, and selected uses the same blue fill as the selected
memory row. Pressing a button emits `Message::SpeedTierChanged(tier)`;
the handler in `app/mod.rs` stashes the tier on `DesktopApp::speed_tier`,
resolves it through `tier_hz`, and ships `SetStepInterval` + `SetRunMode`
to the worker. The worker clamps once more at `MIN_STEP_INTERVAL = 1ms`
(re-exported from `k580_app::actor`) before it overwrites
`Emulator::step_interval`. The next `select!` iteration re-arms the timer
with the new interval (paced) or slice (burst), so clicking a tier while a
