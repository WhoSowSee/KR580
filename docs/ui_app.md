...
An open dropdown owns the arrow and focus-cycle messages before the memory
editor can see them. Pointer opening starts on `TopMenuFocus::Category` with
`TopMenuIndicator::Hidden`, so the menu does not acquire a keyboard outline
merely from a click. `ArrowUp` / `ArrowDown` switch the indicator to
`ArrowFill`, wrap through the enabled rows, and paint the current row with the
same surface fill as pointer hover, without a blue border. `ArrowLeft` /
`ArrowRight` cyclically open the previous/next dropdown category, skipping the
category-only Settings action, move focus to that category, and set the
indicator to `Hidden`; RAM selection stays unchanged and the blue Tab ring does
not appear.
`Tab` / `Shift+Tab` switch the indicator to `TabRing` and walk one closed ring:
`File category → File rows → MP category → MP rows → View category → View rows
→ Settings category → Help category → Help rows`. Crossing a dropdown category
boundary opens that category and gives its top-level label a blue outline with
`6 px` horizontal and `3 px` vertical breathing room; the next Tab enters its
first row. Settings is a category-only stop: it receives the same blue Tab
outline without opening a dropdown, and Enter opens the Settings dialog.
Tab-focused rows retain the blue outline without a fill. Separators, disabled
`Clear Halt`, and passive labels are omitted from the ring. Enter dispatches
the focused row through its existing `Message` and closes the dropdown.
...
1. **«Список ячеек ОЗУ»** – virtualised memory list with the inline
   ...
   preserves its grab point instead of recentering it under the pointer. The
   first 12 logical pixels of a drag use a smooth precision curve,
...
The SVG sources live under `crates/ui/assets/icons/actions/` and are embedded at
...
|---|---|---|---|---|
| Монитор | `device_monitor` (`crates/ui/assets/icons/devices/monitor.svg`) | green | Отобразить монитор | `Ctrl+M` |
...
32×32 with a tooltip on hover). In detached mode the full-width 48 px top
chrome is draggable, including the 16 px outer inset above the controls.
`view/windows.rs` places that drag surface below the rendered panel, so header
buttons keep priority over window movement. Pressing the title band dispatches
`Message::DetachedWindowDragStart(window_id)`, which calls
`iced::window::drag` for that native window. The OS therefore moves the borderless
monitor independently, including outside the main emulator window and onto
another display.
...
translations. The icons are bundled SVGs in `crates/ui/assets/icons/actions/` –
...
The restricted `image/png` codec handles the save-image path without a separate
direct PNG dependency.
...
On the Properties Paper tab, Size, Source, and Orientation form an unframed
column. Their labels and controls align with the driver-specific fields
below, including the first orientation radio. This shared layout applies
to every printer; the Profiles and Preview panels keep their own borders.

Advanced parameter rows keep every text input on the same left edge as
the neighboring full-width selectors. The inline Apply button occupies
space inside the control column, reducing the input width without moving
the field toward its label.
...
но активный `A/B/C` chip снова получает стандартный
`active selection-blue token` без переопределения alpha — тот же спокойный
цвет выбора, что у ячеек РОН мультиплексора и ОЗУ. Активная цель хранится как
`RegisterInlineTarget`, поэтому выбор `B` в верхнем буферном блоке не
подсвечивает `B` в мультиплексоре и наоборот.

`Tab` / `Shift+Tab` обходят верхние `A → B → C` и РОН мультиплексора
`B → C → D → E → H → L` как одно замкнутое кольцо, независимо от того,
открыт ли inline-редактор. Для `A/B/C` стрелки `Left` / `Right` двигают
выбор только внутри этой тройки, а `Up` / `Down` игнорируются и больше не
переносят фокус в мультиплексор. В самом мультиплексоре четыре стрелки
остаются локальной навигацией по сетке `3×2`.
...
показывает базовый T1 следующего M1 fetch: `F2`, `SYNC`, `READY`.
`INTE` больше не подсвечивается искусственно – он отражает реальный
`cpu.interrupt_enable`. Во время `step_tact` лампы читают активный
`MachineCycleKind` и T-фазу внутри этого M-цикла.

| Лампа | Что значит на 8080            | Из чего берём                                   |
|-------|-------------------------------|------------------------------------------------|
| F2    | Такт фазы 2                   | idle: горит; active: нечётная линейная фаза    |
| F1    | Такт фазы 1                   | active: чётная линейная фаза                   |
| SYNC  | Защёлка статус-байта (T1)     | idle: горит; active: `t_in_cycle == 1`         |
| READY | Память/IO готовы              | выключается при `WAIT`/HLT                     |
| WAIT  | Процессор ждёт / остановлен   | `cpu.halted` или HLT halt-ack phase            |
| HOLD  | Запрос DMA-захвата            | всегда `false` – DMA не моделируется           |
| INT   | Запрос прерывания             | `cpu.interrupt_request_pending`                |
| INTE  | Прерывания разрешены          | `cpu.interrupt_enable`                         |
| DBIN  | Строб чтения шины данных      | read M-cycle (`M1`, memory/stack/IO read) T2+  |
| WR    | Строб записи                  | write M-cycle (memory/stack/IO write) T2+      |
| HLDA  | Индикатор halt/hold latch     | горит вместе с `WAIT` для HLT; DMA не моделируется |

`HOLD` остаётся тёмным намеренно: внешнего DMA-запроса в core нет.
`HLDA` используется как пользовательский halt-latch индикатор, чтобы
совпасть с поведением оригинального эмулятора, где HLT снимается
через пункт «Снять флаг HLDA».
...
- `view/chips.rs` (~380 строк) – чистые widget-билдеры под одиночные
...
- `view/menu_dropdowns.rs` (~270 строк) – dropdown rows for `Файл`,
  `МП-Система`, `Вид`, and `Справка`, including Tab-focused row borders,
  arrow-focused hover fills, and disabled HLT reset state.
...
fullscreen windows. At the default `1180×720` window size, a flexible spacer
keeps the original wider separation between the left CPU stack and the right
mux/status column. In maximized windows, or when the main window is at least
900 px tall, both columns are kept as one compact group, centered in the
available schematic area with a fixed 72 px gap. The 24 px / 20 px monospace
value rows are unchanged.
...
- **«Д/Ш команд»** – Дешифратор/Шифратор команд, instruction
  ...
  `schematic_readout` (134×60, 11 px подпись), с 20 px моно
  значением для коротких мнемоник (`NOP`, `INR A`, `SUB B`). Строки
  от 6 символов визуально уменьшаются ступенями (`MVI D,d8` = 18 px),
  а длинные мнемоники дополнительно ограничиваются шириной капсулы.
  Все варианты остаются в том же fixed-height value slot, что и
  «Регистр команд» и «Буфер адреса».
...
- **Цикл (M)** – номер текущего M-цикла внутри инструкции, **с 1**.
  Берётся через школьный layout `layout_for(cpu.timing_opcode())` и
...
`step_instruction` довыполнение активного walking-режима, каждый
`step_tact`). В walking-режиме `StepTact` больше не выполняет opcode
на первом такте: PC, регистры, память и I/O коммитятся только на
границе инструкции, а промежуточные кадры показывают активный
`timing_opcode()` и выбранный branch layout.
...
dragging the slider into high values made the program *finish* faster,
not always *animate* faster. Named tiers communicate the real trade-off:
5 and 20 Hz for readable stepping, 120 Hz for fast paced execution, and
the explicit "don't bother animating" opt-in at 1000 Hz.
...
| `SpeedTier::Medium` | Средне   | `MEDIUM_TIER_HZ = 20`     | Readable continuous run without being as slow as single-step observation. |
| `SpeedTier::High`   | Высоко   | `HIGH_TIER_HZ = 120`      | Default. Fast paced execution without switching to burst mode. |
...
`tier_hz` is deliberately deterministic: Slow / Medium / High / Max
resolve to 5 / 20 / 120 / 1000 instructions per second on every
platform. The High tier no longer queries the monitor refresh rate,
because a 120 Hz display made Medium and High visually collapse into
two identical 120 instr/sec positions. The Max tier still means
"saturate the worker, ignore animation".
...
(open/new/import/close, plus HDD deletion) while `dirty` is true. Two buttons – Cancel
...
focus; Enter activates the focused button. Default logical focus paints
Cancel with the active fill but no white outline. Once Tab/Shift+Tab is
used, `discard_modal_keyboard_focus_visible` switches the indication to a
white border: neither keyboard-focused button receives the logical-focus
fill. Enter and pointer input hide that border before activation.

**State:** `discard_modal_focus: DiscardModalButton`,
`discard_modal_keyboard_focus_visible: bool`, `pending_action:
...
error, `PrinterSetupFocus`, `PrinterPropertiesFocus`, and an independent
`focus_visible` keyboard-outline flag on each dialog. Properties changes stay in this draft
...
| ArrowUp / ArrowDown (inline editor focused) | Move to the previous/next address without changing modes, with the same target-value/placeholder behavior as Tab / Shift+Tab. |
...
| ArrowLeft / ArrowRight | While a top dropdown is open, cyclically switch to the previous/next menu category with `TopMenuIndicator::Hidden`, so no blue Tab ring is drawn and RAM focus does not move. Otherwise navigate the active register target where applicable. |
...
It opens in `Content` with logical focus on the first item of the active
category (`LanguageAnchor` on General), while `keyboard_focus_visible` remains
false so no white ring is drawn until keyboard navigation starts.
...
- **Section state** lives on `SettingsDialog`:
  ...
  - `sidebar_focus: SettingsCategory` is the category cursor, independent from
    the active `category`,
  - `keyboard_focus_visible: bool` records whether Tab/Shift+Tab navigation is
    currently responsible for the visible white ring,
...
- `Reset` opens a stack-layer sub-modal whose `Cancel` / `Confirm`
  ...
  with active fill and no white outline; Tab/Shift+Tab sets
  `reset_confirm_keyboard_focus_visible` and replaces that fill indication with
...
refocus the same id", which iced re-renders against the new row while preserving
whether replacement mode was active.
...
Both native windows load the runtime icon from `crates/ui/assets/icons/icon-64.png` via
`include_bytes!`, decode it with the direct PNG-only `image` path, and create
the iced icon from its RGBA buffer. The full icon
fan-out (`16/32/48/64/128/256` PNGs and a multi-resolution `icon.ico`)
lives in `crates/ui/assets/icons/` and is regenerated by the scripts in
...
- `iced` with default features disabled. The workspace keeps the WGPU renderer,
  the tiny-skia fallback, X11/Wayland support, crisp/web colour handling, and
  the Tokio executor; the UI adds `image-without-codecs`, `svg`, `canvas`, and
  `advanced` (`find_focused` and the `operate` task wrapper). The redundant
  thread-pool executor and automatic Linux theme detection stay disabled
  because every application window supplies an explicit theme.
...
- `windows` (Windows only) with the generated COM/GDI/PrintTicket modules used
  by native printer settings. WGPU uses the same crate and expands the resolved
  Windows graph with its graphics modules.
...
- `winresource` (Windows-only build dependency) for the PE icon resource,
  without its unused Cargo metadata TOML parser.
